...
**版本：** Claude Code 2.1.196  
**集成方式：** `~/.claude/settings.json` hooks + 会话记录监听（watcher）
...
| `PreToolUse` | 工具调用前 | → `running`，或预判为 `waiting`，或超时兜底为 `waiting`（见下） | 见下 |
| `PostToolUse` | 工具调用后 | → `running` | 后台异步 |
| `Stop` | 会话结束 | → `done`，同时清理 `claude-pending` 标记目录 | 同步 |

> 早期版本曾使用 `Notification` hook 做关键词猜测来检测等待状态，会对 Claude 的常规系统通知（更新提示等）产生误判，已移除。

### `PermissionRequest` 是个陷阱：它从未真正触发过

排查中发现：`PermissionRequest` 这个 hook 名字虽然在 Claude Code 二进制的字符串表里存在，配置也完全正确，**但在实际使用中从未触发过一次**——抽查用户全部历史会话（涵盖 500+ 次 `PostToolUse`、上千次工具调用），`PermissionRequest` 出现次数是 **0**。同期 `PreToolUse`/`PostToolUse`/`Stop`/`UserPromptSubmit` 都按预期正常触发。

进一步用调试 hook 抓取了 `PreToolUse` 实际收到的 stdin payload（字段包括 `session_id`、`permission_mode`、`tool_name`、`tool_input` 等），确认里面**没有任何字段**能提前判断"这次调用是否会弹出确认框"——Claude Code 内部判断是否需要人工确认（比如命令包含 heredoc 等"无法静态分析"的语法）是在 hook 触发**之后**单独运行的，hooks 完全看不到这个判断结果，也等不到一个"已经在等你确认"的事件。

**结论：** 没有可靠的 hook 信号能告诉我们"Claude 现在正在等你按 Yes/No"。`settings.json` 中已移除 `PermissionRequest` 配置项（保留它没有意义）。

### 替代方案：预判 + 超时兜底（A+B）

既然没有官方事件可用，改用启发式方法，由 `PreToolUse` hook（`agent-beacon-claude-pretooluse`）实现：

**B（预判）：** 检查即将执行的 Bash 命令文本，如果包含 heredoc（`<<EOF`、`<<'EOF'` 等）或进程替换（`<(...)`、`>(...)`）等 Claude Code 已知会判定为"无法静态分析"的语法，**立即**设为 `waiting`，抢在确认框真正弹出之前。

**A（超时兜底）：** 同时为这次工具调用启动一个 4 秒的后台计时器。如果 4 秒内 `PostToolUse` 没有跟上（绝大多数预授权工具几秒内远不止完成），则推断"大概率卡在确认框等你"，设为 `waiting`。`PostToolUse` 正常触发时会清除计时器对应的标记文件，取消这次兜底判断。

两者都不是 100% 精确：B 只覆盖已知的语法类别，权限系统拒绝的其他原因（危险命令、未授权工具等）覆盖不到；A 对于本身就跑得慢、但已被预授权的命令（比如 `pip install`、长时间脚本）存在误判成"等待中"的风险。这是当前没有官方事件可用情况下的权衡结果。

**拒绝场景的额外处理：** 如果用户拒绝了一个由 B 或 A 判定为 `waiting` 的工具调用，`PostToolUse` 不会触发（工具压根没执行），标记文件不会被正常清除。`agent-claude-watcher`（见下）检测到 Claude 恢复生成纯文本时，会顺手清空所有待处理标记，避免那个本该已经取消的 4 秒超时之后又突然把状态误判回 `waiting`。
...
**② "等待权限"被覆盖回"运行中"：** Claude 生成一条包含 `tool_use`（如调用 Edit/Bash 工具）的消息 → 这条消息会被同时写进会话记录、并交给 `PreToolUse` hook 处理（可能判定为 `waiting`，见上面"预判 + 超时兜底"）→ watcher 轮询到同一条消息，如果不做任何过滤，会把它当作"Claude 在工作"直接设为 `running`，把刚判定的 `waiting` 覆盖掉。

  修复：watcher 检查 assistant 消息内容，**只要包含 `tool_use` 块就跳过**，不设置 `running`。这类消息的状态转换完全交给 `PreToolUse`/`PostToolUse` 处理；watcher 只负责"纯文本"消息——也正是拒绝权限后 Claude 继续说话、但没有任何 hook 能捕捉的那种场景，这与 watcher 的设计初衷一致。

> Codex 的会话记录里工具调用是独立的 `function_call` 条目类型，不会出现在 `role:"assistant"` 的消息里，因此 Codex watcher 天然不受第②种竞态影响，无需同样的过滤。Codex 是否也存在"`PermissionRequest` 从不触发"的同款问题尚未专门验证——目前 Codex 仍配置了该 hook 作为尽力而为的兜底，必要时可比照 Claude Code 的方案做同样的 A+B 替换。
